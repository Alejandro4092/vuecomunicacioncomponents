    ...
    <div>
        ...
        <div>
            <form v-on:submit.prevent="createComic()">
                ...
                <button>Comic  Favorito</button>
            </form>
        </div>

  
    <div id="comics" v-for="(comic, index) in comics" :key="comic">
        <h2 style="color:red" v-if="comicFavorito===comic">¡Favorito!{{comicFavorito.titulo}}</h2>
        <ComicComponent :comic="comic" v-on:seleccionarFavorito="seleccionarFavorito" :index="index" v-on:deleteComic="deleteComic"/>
